...
def get_mask_location(model_type, category, model_parse: Image.Image, keypoint: dict, width=768, height=1024):
    im_parse = model_parse.resize((width, height), Image.NEAREST)
    parse_array = np.array(im_parse)

    if model_type == 'hd':
        arm_width = int(60 * height / 512)
    elif model_type == 'dc':
        arm_width = int(45 * height / 512)
    else:
    ...
    if category == 'dresses' or category == 'upper_body':
        ...
        arm_mask = cv2.dilate(np.logical_or(im_arms_left, im_arms_right).astype('float32'), np.ones((5, 5), np.uint16),
                              iterations=4)
